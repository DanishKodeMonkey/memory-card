export async function fetchCatImages() {
    const validStatusCodes = [
        100, 101, 102, 103, 200, 201, 202, 203, 204, 205, 206, 207, 208, 300,
        301, 302, 303, 304, 305, 307, 308, 400, 401, 402, 403, 404, 405, 406,
        407, 408, 409, 410, 411, 412, 413, 414, 415, 416, 417, 418, 420, 421,
        422, 423, 424, 425, 426, 428, 429, 431, 444, 450, 451, 497, 498, 499,
        500, 5011, 502, 503, 504, 506, 507, 508, 509, 510, 511, 521, 522, 523,
        525, 530, 599,
    ];
    // randomly generate an array of 8 HTTP numbers
    const generateRandomStatusCodes = () => {
        const statusCodes = [];
        while (statusCodes.length < 8) {
            // pick a random index item from the array of valid HTTP codes
            const randomIndex = Math.floor(
                Math.random() * validStatusCodes.length
            );
            const code = validStatusCodes[randomIndex];
            // Ensure statscode has not already been taken.
            if (!statusCodes.includes(code)) {
                statusCodes.push(code);
            } else {
                console.log(code, ' already exists, retrying...');
            }
        }
        // return final list of random statusCodes
        return statusCodes;
    };
    const randomStatusCodes = generateRandomStatusCodes();
    const catImageURLs = randomStatusCodes.map(
        (code) => `https://http.cat/${code}`
    );

    return catImageURLs;
}
